...
4. **Verify your TLS fingerprint before blaming sensor payloads.** Akamai flags mismatched JA3/JA4 at the edge before it ever evaluates sensor content. Hyper requires the **same TLS/client for the entire session**. **J1m’s Bot charge path:** one **undici** client warm→sensors→PDP→GraphQL (Lovable→Fly `#40` / live ISP `cart_get` JSON 200). Do **not** default to tls-worker park/handoff — that split Ghost-denied GraphQL after get-token. Opt-in only: `sensorTls` / `apiTls`.
