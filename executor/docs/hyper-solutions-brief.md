...
4. **Verify your TLS fingerprint before blaming sensor payloads.** Akamai flags mismatched JA3/JA4 at the edge before it ever evaluates sensor content. Use `tls-client` or `azuretls-client`; do not use native `axios`, `node-fetch`, or the built-in `https` module. Hyper requires the **same TLS client for the entire session**. **J1m’s Bot:** ISP uses `tls-worker` chrome_131 for warm+sensors, parks that session, undici for PDP (tls PDP Ghosts), then **reuses the parked worker** for api.*. That hybrid clears `_abck` + PDP HTML but still GraphQL-Ghosts — see `HYPER_HAR_ALIGNMENT.md`. Do not treat `bodySuccess=true` + plateau `ind=-1` as SoftBlock alone.
