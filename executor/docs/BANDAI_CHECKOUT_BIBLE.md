...
| GE charge POSTs | **`/1` network; `/2` fulfill local; first `/3+` to issuer once** | Bank silent after 14:09 when `/3` was fulfilled local (only `/2` on wire). `/3` is authorize; `/2`+`/3` was the Revolut pair. |
