...
class FeedHub {
  ...
  authOk(req) {
    // Default open = same as desktop license today: any non-empty API key.
    // Optional allowlist via MONITOR_AUTH_MODE=allowlist + MONITOR_API_KEYS.
    const mode = String(process.env.MONITOR_AUTH_MODE || "open").toLowerCase();
    const token = extractToken(req);
    if (mode === "open") return Boolean(token);

    const keys = String(process.env.MONITOR_API_KEYS || "")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
    if (!keys.length) return Boolean(token);
