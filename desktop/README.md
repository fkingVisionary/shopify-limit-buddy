...
|----------|---------|----------|
| Mode **Autocheckout** | `bandaiMode=checkout` | Login → ATC → cart hold (HTTP + F5) → pay |
| Mode **ATC only** | `bandaiMode=atc` | Login → ATC → cart hold only (~30 min window + countdown / Retry pay) |
