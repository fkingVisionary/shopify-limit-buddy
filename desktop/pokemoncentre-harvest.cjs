// Pokémon Centre AU harvest pool (desktop main process).
// Disney/Toymate-shaped: serializable Reese+DD jar (+ optional hCaptcha TTL).
// Empty bank → PC checkout cold-starts edge warm on the critical path.
...
function createPokemonCentreHarvestPool({ sidecar, emit } = {}) {
  /** @type {PcHarvestSession[]} */
  let pool = [];
  let running = false;
  let busy = false;
  let paused = false;
  ...
  function isFresh(s, t = now()) {
    if (!s || Number(s.edgeExpiresAt) <= t) return false;
    const reese = s.cookies?.reese84 || "";
    const dd = s.cookies?.datadome || "";
    return String(reese).length > 20 && String(dd).length > 8;
  }
  ...
  function snapshot() {
    ...
    return {
      ...
      sessions: fresh.map((s) => ({
        ...
        edgeTtlSec: Math.max(0, Math.round((Number(s.edgeExpiresAt) - t) / 1000)),
  ...
  function publish() {
    emit?.({ type: "pokemoncentreHarvest", data: snapshot() });
  }
  ...
  async function harvestOne(entries) {
    if (!sidecar?.status?.().running) {
      lastError = "Start the engine first (Harvest needs Hyper via local executor)";
      publish();
    ...
    publish();
    try {
      const res = await sidecar.harvestPokemonCentre({
        proxy,
      ...
      if (!res?.ok || !res.session) {
        ...
        publish();
        return { ok: false, error: lastError, isIpBanned: Boolean(res?.isIpBanned) };
      }
      const s = res.session;
      /** @type {PcHarvestSession} */
      const row = {
        id: s.id || `pch_${crypto.randomBytes(4).toString("hex")}`,
        proxy: s.proxy || proxy,
        ...
        edgeExpiresAt: s.edgeExpiresAt || now() + 3 * 60_000,
        captchaExpiresAt:
        ...
        locale: s.locale || config.locale || "en-au",
        origin: s.origin || "https://www.pokemoncenter.com",
        transport: s.transport || null,
      };
      if (!isFresh(row)) {
        failedCount += 1;
        lastError = "harvested session missing reese84/datadome";
        publish();
        return { ok: false, error: lastError };
      }
      pool.push(row);
  ...
  function start({ proxyGroupId, desired, solveCaptcha, locale, getEntries } = {}) {
    ...
    if (locale) config.locale = String(locale);
    running = true;
    paused = false;
    lastError = null;
    publish();
  ...
  /** Keep bank; skip refill while checkout lanes are in-flight. */
  function pause() {
    paused = true;
    publish();
    return snapshot();
  ...
  function configure(patch = {}) {
    ...
    if (patch.locale != null) config.locale = String(patch.locale);
    publish();
    return snapshot();
  }

  /**
   * Claim one session for checkout. Prefer captcha-ready when opted in.
   * Single-use. Caller should reclaim (put back) if session proves dead before warm skip.
   */
  function take({ preferCaptcha = false } = {}) {
    evictExpired();
    const t = now();
    let idx = -1;
    if (preferCaptcha) {
  ...
  /** Return a claimed-but-unused session to the bank (dead-slot reclaim). */
  function reclaim(session) {
    if (!session || typeof session !== "object") return snapshot();
    if (!isFresh(session)) {
      publish();
      return snapshot();
    }
    // Avoid dup ids
    pool = pool.filter((s) => s.id !== session.id);
    pool.push(session);
    publish();
    return snapshot();
  }
