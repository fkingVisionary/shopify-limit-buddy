...
} = require("./bandai-harvest-autoarm.cjs");
...
function makeJob(overrides = {}) {
  return {
    ...
    task: {
      ...
      bandaiArea: "au",
      proxyGroupId: "mon-px",
      bandaiHarvestProxyGroupId: "checkout-px",
      ...overrides.task,
    },
  };
}
...
function fakeHarvest(initial = {}) {
  ...
  return {
    ...
    configure(patch = {}) {
      ...
      if (patch.area != null) config.area = patch.area;
    ...
    start(opts = {}) {
      ...
      if (opts.area) config.area = opts.area;
      return this.snapshot();
    },
    stop() {
      calls.stop += 1;
...
test("isBandaiMonitorCheckoutJob requires monitor + checkout on hit", () => {
  assert.equal(isBandaiMonitorCheckoutJob(makeJob()), true);
  assert.equal(
    isBandaiMonitorCheckoutJob(makeJob({ task: { bandaiCheckoutOnHit: false } })),
    false,
  );
  assert.equal(
    isBandaiMonitorCheckoutJob(makeJob({ task: { bandaiMode: "checkout" } })),
    false,
  );
  assert.equal(
    isBandaiMonitorCheckoutJob(makeJob({ task: { bandaiAutoHarvest: false } })),
    false,
  );
});
...
test("auto-arm starts harvest and stops when last ref releases", () => {
  ...
  const auto = createBandaiHarvestAutoArm({
    harvest,
...
test("missing proxy group skips arm", () => {
  ...
  const out = auto.ensureForJobs([
    makeJob({ task: { proxyGroupId: null, bandaiHarvestProxyGroupId: null } }),
  ]);
